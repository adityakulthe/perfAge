...
## Git Configuration - Fixing Commit Author

If you need to fix the author name on the latest commit, follow these steps:

### Steps to Update Commit Author

1. **Verify current git user configuration:**
   ```bash
   git config user.name
   ```

2. **Set the correct author name (if needed):**
   ```bash
   git config user.name "aditya kulthe"
   ```

3. **Amend the last commit to update the author:**
   ```bash
   ...
   - `--reset-author`: Update the author to the current git user configuration
   - `--no-edit`: Keep the existing commit message

4. **Force push the corrected commit:**
   ```bash
   git push --force
   ```
   
   ⚠️ **Warning:** Force pushing rewrites history. Only use this on branches you own or with team coordination.

5. **Verify the commit author has been updated:**
   ```bash
   git log -1 --pretty=format:"%an <%ae>"
   ```
   
   This should now display: `aditya kulthe <email@example.com>`

### Additional Git Configuration

To set the author email as well:
```bash
...
```bash
git config --global user.name "aditya kulthe"
